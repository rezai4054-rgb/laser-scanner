...
# Scan window around the object center (clipped to printer limits).
SCAN_SIZE_X = 40.0  # mm
SCAN_SIZE_Y = 40.0  # mm
...
Z_OBJECT_STEP_MM = 0.2  # mm per sample while finding the object top
...
LINE_STEP_Y_MM = 0.8  # Y pitch between zigzag lines
SAMPLES_PER_SEGMENT = 2  # ESP32 R polls while a short segment is queued

# Cliff / drop search.
DROP_SEARCH_MAX_MM = 30.0  # do not search deeper than this
...
class LaserScanner:
    ...
    def find_object(self) -> None:
        ...
        for _ in range(MAX_OBJECT_SEARCH_STEPS):
            ...
            if nxt <= self.z_bed_focus + 0.05:
                self.head.laser_off()
                raise ScannerError(
                    "object not found before reaching Z_BED_FOCUS; "
                    "stop safely — check height estimate and XY center"
                )
            self.printer.move_abs(z=nxt, feed=CALIBRATE_FEED_MM_MIN)
    ...
    def handle_drop(self) -> str:
        ...
        self.log.info("Signal lost — drop/cliff search from Z=%.3f down to %.3f (max 30 mm)", start_z, limit_z)
    ...
    def raster_scan(self) -> None:
        ...
        if x1 - x0 < SEGMENT_MM or y1 - y0 < LINE_STEP_Y_MM:
            raise ScannerError("scan window too small after clipping to printer limits")

        self.log.info("Raster window X[%.2f..%.2f] Y[%.2f..%.2f] seg=%.2f ystep=%.2f", x0, x1, y0, y1, SEGMENT_MM, LINE_STEP_Y_MM)
        y = y0
        direction = 1.0
        empty_lines = 0
        line_index = 0
        while y <= y1 + 1e-6 and line_index < MAX_SCAN_LINES:
            ...
            # Resume near last tracking height but never below the bed.
                ...
                if lost >= SAMPLES_PER_SEGMENT:
                    ...
                    if result == "surface":
                        if self.record_if_valid(self.sample()):
                            points_this_line += 1
                        continue
                    # empty bed / cliff / edge: stop recording on this region
